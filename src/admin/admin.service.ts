...
export class AdminService {
    constructor(@InjectModel(admin.name) private adminModel: Model<admin>,
                @InjectModel(adminproduct.name) private adminProductModel:Model<adminproduct>,
    ...
    async  Create(req: adminDto){
        ...
        async getProductId(req:adminproductDto){
            try{
                const adminProdId=await this.adminProductModel.findOne({productId:req.productId})
                if(adminProdId){
                    return{
                        statusCode:HttpStatus.OK,
                        prod:adminProdId
     ...
     async updateContact(req:contactDto){
        try{
            const resUpdate=await this.contactModel.updateOne({contactId:req.contactId},{$set:{name:req.name,mailId:req.mailId,phoneNumber:req.phoneNumber}})
            if(resUpdate){
                return{
                    statusCode:HttpStatus.OK,
                    Updateres:resUpdate 
     ...
     async deleteContact(req:contactDto){
        try{
            const delconct=await this.contactModel.deleteOne({contactId:req.contactId})
            if(delconct){
                return{
                    statusCode:HttpStatus.OK,
    ...
    async updatefeedback(req:feedbackDto){
        try{
            const editfeedback=await this.feedbackModel.updateOne({feedbackId:req.feedbackId})
            if(editfeedback){
                return {
                    statusCode:HttpStatus.OK,
    ...
    async deletefeedback(req:feedbackDto){
        try{
            const delfeedback=await this.feedbackModel.deleteOne({feedbackId:req.feedbackId})
            if(delfeedback){
                return{
                statusCode:HttpStatus.OK,
                res:delfeedback
    ...
    async feedbackbyId(req:feedbackDto){
        try{
            const resfeedId=await this.feedbackModel.find({feedbackId:req.feedbackId})
            if(resfeedId){
                return{
                    statusCode:HttpStatus.OK,
                    feedId:resfeedId
    ...
    async deletecomplaint(req:complaintDto){
        try{
            const deleteComp =await this.complaintModel.deleteOne({complaintId:req.complaintId})
            if(deleteComp){
                return{
                    statusCode:HttpStatus.OK,
    ...
    async getComplaintByid(req:complaintDto){
        try{
            const resId=await this.complaintModel.find({complaintId:req.complaintId})
            if(resId){
                return{
                    statusCode:HttpStatus.OK,
                    resComp:resId
    ...
    async editComplaint(req:complaintDto){
        try{
            const editComp=await this.complaintModel.updateOne({complaintId:req.complaintId})
            if(editComp){
                return{
                    statusCode:HttpStatus.OK,
                    updateComp:editComp
    ...
    async getpointById(req:rewardpointDto){
        try{
            const getPointsRes=await this.rewardpointModel.find({rewarId:req.rewardId})
            if(getPointsRes){
                return{
                    statusCode:HttpStatus.OK,
    ...
    async deletePoints(req:rewardpointDto){
        try{
            const delpoint=await this.rewardpointModel.deleteOne({rewardId:req.rewardId})
             if(delpoint){
                return{
                    statusCode:HttpStatus.OK,
                    Message:'deleted Sucessfully',
    ...
    async editPoints(req:rewardpointDto){
        try{
            const editRes=await this.rewardpointModel.updateOne({rewardId:req.rewardId})
            if(editRes){
                return{
                statusCode:HttpStatus.OK,
                pointEdit:editRes

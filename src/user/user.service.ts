...
export class UserService {
  ...
  async create(params: userDto, image?: any): Promise<any> {
    try {
      ...
      if (params.role === 'service') {
        (userObject.serviceId = params.serviceId),
          (userObject.name = params.name),
          (userObject.mobileNumber = params.mobileNumber),
          (userObject.email = params.email),
          (userObject.rating = params.rating),
  ...
  async loginUser(req: userDto) {
    try {
      ...
      const loginRes = await this.UserModel.findOne({
  ...
  async editUser(req: userDto) {
    try {
      const editRes = await this.UserModel.updateOne(
        { userId: req.userId },
        {
          $set: {
  ...
  async updateVendor(req: userDto, image) {
    try {
      ...
      const updateVen = await this.UserModel.updateOne(
  ...
  async deleteVendor(req: userDto) {
    try {
      const delVendor = await this.UserModel.deleteOne({
        vendorId: req.vendorId,
      });
      if (delVendor) {
        return {
  ...
  async deleteProvider(req: userDto) {
    try {
      const delprovider = await this.UserModel.deleteOne({
  ...
  async updateServiceProvider(req: userDto, image) {
    try {
      ...
      const updateService = await this.UserModel.updateOne(
        { providerId: req.providerId },
        {
          $set: {
            userName: req.userName,
  ...
  async updateRole(id: string, role: string) {
    try {
      const roleResp = await this.UserModel.findById(
        { id: id },
        { $set: { role } },
        { new: true },
      );
  ...
  async getService(req: userDto) {
    try {
      const serviceResp = await this.UserModel.findOne({
  ...
  async editService(params: userDto) {
    try {
      const updateService = await this.UserModel.updateOne(
